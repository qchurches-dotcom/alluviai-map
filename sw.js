...
const SHELL='alluviai-shell-v7', TILES='alluviai-tiles-v1';
...
self.addEventListener('install',e=>{e.waitUntil(caches.open(SHELL)
  .then(c=>Promise.allSettled(SHELL_FILES.map(f=>c.add(f)))).then(()=>self.skipWaiting()));});
...
function isTile(u){return /arcgisonline|tile\.openstreetmap|base\.maps\.vic|opendata\.maps\.vic|basemaps|\/tile\//.test(u);}
